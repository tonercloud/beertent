...
//  was function updateFilterModel(name) {
function updateFilterModel(brewery_name) {
    if (filter.length > 0) {
        for (var i = filtermodel.count-1; i >= 0; i--) {
            if (!checkFilter(filter, filtermodel.get(i).brewery_name))
...
function updateUniqueChars() {
    ...
    // console.log("search.js 41 filtermodel", filtermodel);
    ...
    if (filtermodel.count > 1) {
        ...
        for (var i = 0; i < filtermodel.count; i++) {
            var name = filtermodel.get(i).brewery_name;
            // console.log("search.js 50 name etc", name);
